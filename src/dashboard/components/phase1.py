...
def render_phase1(ret_df):
    ...
    avg_conf = ret_df["avg_retrieval_confidence"].max() if "avg_retrieval_confidence" in ret_df.columns else 0
    col5.metric("Best Avg Confidence", f"{avg_conf:.4f}")
    ...
    if raw_ret_rows:
        ...
        tab1, tab2, tab3 = st.tabs(["Hit Rate by Question", "Retrieval Confidence", "Data Table"])
        with tab1:
            pivot = pq_df.pivot_table(index="question", columns="chunk_size", values="hit", aggfunc="mean")
            st.bar_chart(pivot)
        with tab2:
            conf_pivot = pq_df.pivot_table(
                index="question", columns="chunk_size", values="mean_retrieval_score", aggfunc="mean"
            )
